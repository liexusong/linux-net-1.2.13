...
void ip_rt_add(
    short flags,
    unsigned long dst,    // 目标IP地址
    unsigned long mask,   // 子掩码
    unsigned long gw,     // 网关IP
    struct device *dev,   // 路由对应的设备
    ...
    {
        if (!((dst ^ dev->pa_addr) & dev->pa_mask)) { // match
            mask = dev->pa_mask;
            flags &= ~RTF_GATEWAY;
...
static int rt_new(struct rtentry *r)
{
    ...
    flags = r->rt_flags;
    daddr = ((struct sockaddr_in *) &r->rt_dst)->sin_addr.s_addr;    // 路由对应的IP
    mask = ((struct sockaddr_in *) &r->rt_genmask)->sin_addr.s_addr; // 路由对应的子掩码
    gw = ((struct sockaddr_in *) &r->rt_gateway)->sin_addr.s_addr;   // 网关IP地址
    ...
    if (!dev && (flags & RTF_GATEWAY)) { // 如果设置网关, 那么找到网关对应的设备对象
        struct device *dev2;

        for (dev2 = dev_base ; dev2 != NULL ; dev2 = dev2->next) {
            if ((dev2->flags & IFF_UP) && dev2->pa_addr == gw) { // 如果网关IP有对应的设备, 那么就将起转换成非网关
                flags &= ~RTF_GATEWAY;
                dev = dev2;
                break;
            }
...
struct rtable *ip_rt_route(unsigned long daddr, struct options *opt, unsigned long *src_addr)
{
    ...
    for (rt = rt_base; rt != NULL || early_out ; rt = rt->rt_next) {
         ...
         */
        if (rt->rt_flags & RTF_GATEWAY) // 跳过网关
            continue;
