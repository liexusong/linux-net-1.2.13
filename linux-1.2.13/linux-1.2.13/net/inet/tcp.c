...
static __inline__ void tcp_set_state(struct sock *sk, int state)
{
    ...
    if (state == TCP_ESTABLISHED && sk->state == TCP_SYN_RECV) {
        wake_up_interruptible(&master_select_wakeup); // 唤醒master_select_wakeup的进程
    }
    sk->state = state;
 ...
 */
// 关闭待连接套接字(就是已经在监听socket的接收队列中, 但还没有被应用层accept的套接字)
...
static void tcp_time_wait(struct sock *sk)
{
    ...
    if (!sk->dead)
        sk->state_change(sk);
    reset_msl_timer(sk, TIME_CLOSE, TCP_TIMEWAIT_LEN); // 重置定时器
}
...
void tcp_do_retransmit(struct sock *sk, int all)
{
    ...
    prot = sk->prot;
    skb = sk->send_head; // 当前socket的发送队列
    ...
    while (skb != NULL) {
        ...
        iph->id = htons(ip_id_count++); // 分配新的IP包ID (区别之前的数据包)
        ip_send_check(iph);             // 更新IP包的校验和
        ...
        th->ack_seq = ntohl(sk->acked_seq);                  // 回复序列号
        th->window = ntohs(tcp_select_window(sk));           // 更新接收窗口大小
        tcp_send_check(th, sk->saddr, sk->daddr, size, sk);  // 更新TCP包的校验和

        /*
         *    If the interface is (still) up and running, kick it.
         */

        if (dev->flags & IFF_UP) { // 如果发送数据包的设备处于工作状态
            /*
            ...
            if (sk && !skb_device_locked(skb)) {
                ...
                dev_queue_xmit(skb, dev, sk->priority); // 把数据包发送出去
            }
        }
...
void tcp_retransmit_time(struct sock *sk, int all)
{
    ...
    sk->retransmits++;
    sk->backoff++;
    sk->rto = min(sk->rto << 1, 120*HZ); // 退避算法(每次增加1倍), 但不能超过2分钟
...
static void retransmit_timer(unsigned long data)
{
    ...
    // ack_backlog 大于 0 表示还有没回复ack的包
    // zapped 等于 0 表示连接没有被重置
         ...
         */
    {
        ...
        skb = sk->send_head; // 未得到对端应答的数据包列表
        if (!skb) {
            restore_flags(flags);
        }
        else
    ...
    case TIME_KEEPOPEN: // 保活超时
        ...
        if (sk->prot->write_wakeup)
              sk->prot->write_wakeup(sk); // 发送保活数据包
        sk->retransmits++;
        tcp_write_timeout(sk);
        break;
             ...
             unsigned long saddr, struct inet_protocol *protocol)
{
    struct tcphdr *th;
    struct sock *sk;
    struct iphdr *iph = (struct iphdr *)header; // IP头部

    header += 4*iph->ihl;

    th = (struct tcphdr *)header; // TCP头部
...
static int tcp_listen_select(struct sock *sk, int sel_type, select_table *wait)
{
    if (sel_type == SEL_IN) {
        ...
        sk->inuse = 1;
        retval = (tcp_find_established(sk) != NULL); // 从接收队列中获取一个skb
        release_sock(sk);
        if (!retval)
...
static void tcp_send_skb(struct sock *sk, struct sk_buff *skb)
{
    ...
    if (size == sizeof(struct tcphdr)) { // 没有内容的数据包
    ...
    // 对端应该回复的应答序列号，如果对端应答的序列号一致，即把缓存从send_head中删除
    ...
    // 2. 连接已经超时并且重发数据了
    // 3. 发送出去的数据包数量大于限制
    if (after(skb->h.seq, sk->window_seq)  // seq > window_seq (当前序列号大于对端的窗口长度)
        ...
        || sk->packets_out >= sk->cong_window)
    {
         ...
         */
        // 启动窗口探测定时器
        if (before(sk->window_seq, sk->write_queue.next->h.seq) // 1. 当前发送的数据包超出了对端的窗口长度
            && sk->send_head == NULL                            // 2. 对端已经应答本地所有的的数据包
            && sk->ack_backlog == 0)                            // 3. 已经对对端所有的数据包都作出应答
            reset_xmit_timer(sk, TIME_PROBE0, sk->rto);         // 4. 启动窗口探测定时器
    }
    else
    {
        /*
         ...
         */
        th->ack_seq = ntohl(sk->acked_seq);        // 应答ack序列号
        th->window = ntohs(tcp_select_window(sk)); // 窗口大小

        tcp_send_check(th, sk->saddr, sk->daddr, size, sk); // 计算TCP校验和

        sk->sent_seq = sk->write_seq; // 更新发送出去的序列号(发送到网络中)
         ...
         */
        // 将会调用: ip_queue_xmit()
        sk->prot->queue_xmit(sk, skb->dev, skb, 0); // 直接发送出去(这个函数会把skb放置到sk的send_head队列中)
         ...
         */
        reset_xmit_timer(sk, TIME_WRITE, sk->rto); // 启动写超时定时器
    }
}
...
void tcp_enqueue_partial(struct sk_buff * skb, struct sock * sk)
{
     ...
     */
    sk->partial_timer.expires = HZ; // 1秒
...
static void tcp_send_ack(unsigned long sequence,
                         unsigned long ack,
                         struct sock *sk,
                         struct tcphdr *th,
                         unsigned long daddr)
{
     ...
     */
    // 调用: ip_build_header()
    tmp = sk->prot->build_header(buff, sk->saddr, daddr,
    ...
    t1->dest = th->source;     // 设置目标端口
    t1->source = th->dest;     // 设置源端口
    t1->seq = ntohl(sequence); // 设置序列号
    t1->ack = 1;               // 设置ACK标志
    sk->window = tcp_select_window(sk);
    t1->window = ntohs(sk->window); // 设置窗口大小
    t1->res1 = 0;
    t1->res2 = 0;
    t1->rst = 0;
    t1->urg = 0;
     ...
     *    to a keepalive.
     */
    if (ack == sk->acked_seq) { // 如果要应答的序列号等于期望从远端接收的下一个序列号(表示所有数据包都已经被应答了)
        sk->ack_backlog = 0;
        sk->bytes_rcv = 0;
        sk->ack_timed = 0;
    ...
    sk->prot->queue_xmit(sk, dev, buff, 1); // 把数据包发送出去
}
                     ...
                     int len, int nonblock, unsigned flags)
{
    ...
    while (len > 0) {             // 直到把from数据全部发送出去
        if (sk->err) {            /* Stop on an error */ // 如果有错误退出
            release_sock(sk);
            if (copied)
                return copied;
            tmp = -sk->err;
        ...
        if (sk->shutdown & SEND_SHUTDOWN) { // 如果socket被关闭了
            release_sock(sk);
            sk->err = EPIPE;
            if (copied)
                return(copied);
         ...
         */
        if ((skb = tcp_dequeue_partial(sk)) != NULL) { // 如果partial队列不为空
            int hdrlen;

            /* MAC头部 + IP头部 + TCP头部 */
            hdrlen = (unsigned long)skb->h.th
                      - (unsigned long)skb->data
                      + sizeof(struct tcphdr);

            /* Add more stuff to the end of skb->len */
            if (!(flags & MSG_OOB)) { // 如果不是紧急数据
                ...
                copied += copy;
                len -= copy;
                sk->write_seq += copy; // 更新写到缓冲区的最大序列号
            }
        ...
        copy = sk->window_seq - sk->write_seq; // 窗口还有多少空间?
     ...
     */
        ...
        send_tmp = NULL;
        if (copy < sk->mss && !(flags & MSG_OOB)) { // 如果要发送的数据小于TCP段的大小, 并且不是紧急数据
            /*
         ...
         */
        // 构建IP头部和MAC头部
        tmp = prot->build_header(skb, sk->saddr, sk->daddr,
                                 &dev, IPPROTO_TCP, sk->opt,
        ...
        if (flags & MSG_OOB) { // 如果是紧急数据, 设置紧急标志
 ...
 */
// 发送一个空数据的ACK包(告诉对端自己的窗口大小, 触发对端发送过程)
static void tcp_read_wakeup(struct sock *sk)
{
    ...
    if (!sk->ack_backlog) // 未发送应答的数据包数量(如果是0表示全部数据包都应答了)
        return;
...
static void cleanup_rbuf(struct sock *sk)
{
    ...
    left = sk->prot->rspace(sk); // 读缓冲区剩余空间
    ...
    if ((rspace = sk->prot->rspace(sk)) != left) { // 有更多空间了
        ...
        if (rspace > (sk->window - sk->bytes_rcv + sk->mtu)) {
            ...
            tcp_read_wakeup(sk); // 告诉对端窗口已经有足够的空间, 可以发送数据
        }
        else
        {
                    ...
                    int len, int nonblock, unsigned flags)
{
    ...
    found_ok_skb:
        ...
        *seq += used;
        ...
        memcpy_tofs(to,
            ((unsigned char *)skb->h.th) + skb->h.th->doff * 4 + offset, used);
...
static int tcp_close_state(struct sock *sk, int dead)
{
     ...
     *    reset mistake.
     */
    if (dead && ns == TCP_FIN_WAIT2) { // TCP_FIN_WAIT2状态需要设置TIME_CLOSE定时器
...
static void tcp_send_fin(struct sock *sk)
{
     ...
     */
    // 构建IP头部和MAC头部
    tmp = prot->build_header(buff,sk->saddr, sk->daddr,
                             &dev, IPPROTO_TCP, sk->opt,
                             sizeof(struct tcphdr), sk->ip_tos,
                             sk->ip_ttl);
    if (tmp < 0) {
        int t;
          /*
...
void tcp_shutdown(struct sock *sk, int how)
{
    ...
    if (tcp_close_state(sk, 0)) // 更改socket的状态
        tcp_send_fin(sk);       // 如果需要发送FIN包给对端
...
static void tcp_reset(unsigned long saddr, unsigned long daddr,
                      ...
                      int tos, int ttl)
{
    ...
    tmp = prot->build_header(buff, saddr, daddr,
                             &ndev, IPPROTO_TCP, opt,
...
tcp_conn_request(
    ...
    newsk->shutdown = 0;
    newsk->ack_backlog = 0;
    newsk->acked_seq = skb->h.th->seq + 1;  // 已确认序列号
    newsk->copied_seq = skb->h.th->seq + 1; // 已复制序列号
    newsk->fin_seq = skb->h.th->seq;
    newsk->state = TCP_SYN_RECV; // 新创建的 socket 状态为: TCP_SYN_RECV, 因为还需要三次握手，还需要一个ack包变为 ESTABLISHED 状态
    newsk->timeout = 0;
    ...
    put_sock(newsk->num, newsk); // 把 socket 放到连接池中
    ...
    buff->len += tmp;
    t1 = (struct tcphdr *)((char *)t1 + tmp); // TCP头部
    ...
    newsk->prot->queue_xmit(newsk, ndev, buff, 0); // 把ACK+SYN包发送出去
    reset_xmit_timer(newsk, TIME_WRITE , TCP_TIMEOUT_INIT);
    skb->sk = newsk; // 把新的socket绑定到skb
    ...
    skb_queue_tail(&sk->receive_queue, skb); // 把 skb 添加到 listen socket 的 receive_queue 队列中
    sk->ack_backlog++;
    release_sock(newsk);
    tcp_statistics.TcpOutSegs++;
}
...
extern __inline__ int tcp_ack(struct sock *sk, struct tcphdr *th,
                              unsigned long saddr, int len)
{
    ...
    ack = ntohl(th->ack_seq); // ack序列

    if (ntohs(th->window) > sk->max_window) { // 扩大发送窗口
     ...
     */
    // 1. ack > sent_seq: 如果收到的确认ack大于发送出去的序列(错误ack包)
    // 2. ack < rcv_ack_seq: 如果收到的确认ack小于之前收到的确认序列(忽略)
...
int tcp_rcv(struct sk_buff *skb, struct device *dev,
            struct options *opt, unsigned long daddr,  // 目标IP地址
            unsigned short len, unsigned long saddr,   // 源IP地址
            int redo, struct inet_protocol *protocol)
{
    ...
    if (skb->pkt_type != PACKET_HOST) { // 丢弃不是发送给我们的包
        kfree_skb(skb, FREE_READ);
        return(0);
    }
    ...
    if (!redo) { // 从 ip_rcv() 调用时, 此参数为0
        ...
        th->seq = ntohl(th->seq); // 序列号

        /* See if we know about the socket. */
        if (sk == NULL) { // 如果当前IP和端口没有对应的socket, 那么把这个包释放
            /*
        ...
        cli();
        if (sk->inuse) { // 正在处理中, 所以先把 buffer 放到 back_log 队列中
    ...
    {
        if (sk == NULL) {
            tcp_reset(daddr, saddr, th, &tcp_prot,
     ...
     */
    // TCP状态机处理
    if (sk->state != TCP_ESTABLISHED) { /* Skip this lot for normal flow */ // 如果socket状态不是可用的
        /*
         *    Now deal with unusual cases.
         */

        if (sk->state == TCP_LISTEN) { // 如果 socket 是 listen 状态（说明这是一个新建的连接，所以需要进行三次握手）
            // 三次握手的第一个包必须是 syn 包
            ...
            tcp_conn_request(sk, skb, daddr, saddr, opt, dev, tcp_init_seq()); // 创建一个新的 socket 连接
        ...
        if (sk->state == TCP_SYN_RECV
            && th->syn
            && th->seq+1 == sk->acked_seq) // 这个状态需要接收一个ack包
        {
            kfree_skb(skb, FREE_READ);
            release_sock(sk);
            return 0;
        ...
        if (sk->state == TCP_SYN_SENT) { // 调用 connect() 后会进入这个状态, 需要接收一个ack包
            /* Crossed SYN or previous junk segment */
            if (th->ack) {
                /* We got an ack, but it's not a good ack */
                if (!tcp_ack(sk, th, saddr,len)) { // 如果是一个不合法的ack
                    /* Reset the ack - its an ack from a
                       different connection  [ th->rst is checked in tcp_reset()] */
                    tcp_statistics.TcpAttemptFails++;
                    tcp_reset(daddr, saddr, th, sk->prot,
                              opt, dev, sk->ip_tos, sk->ip_ttl); // 发送一个reset包给对端
                    kfree_skb(skb, FREE_READ);
                    release_sock(sk);
                    return(0);
                }
...
#define BSD_TIME_WAIT
    ...
    if (th->ack && !tcp_ack(sk,th,saddr,len)) {
         ...
         */
        if (sk->state == TCP_SYN_RECV) {
            tcp_reset(daddr, saddr, th,sk->prot, opt,
                      dev, sk->ip_tos, sk->ip_ttl);
        }
        kfree_skb(skb, FREE_READ);
        release_sock(sk);
        return 0;
...
static void tcp_write_wakeup(struct sock *sk)
{
     ...
     */
    sk->prot->queue_xmit(sk, dev, buff, 1); // 把包发送出去
