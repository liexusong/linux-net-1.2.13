    ...
    int len, int noblock, unsigned flags, struct sockaddr_in *usin, int addr_len)
{
    ...
    tmp = sk->prot->build_header(skb, sk->saddr, sin.sin_addr.s_addr,
			                     &dev, sk->protocol, sk->opt, skb->mem_len,
			                     sk->ip_tos, sk->ip_ttl);
    if (tmp < 0) {
...
static int raw_write(struct sock *sk, unsigned char *buff,
...
int raw_recvfrom(struct sock *sk, unsigned char *to,
    			 int len, int noblock, unsigned flags,
