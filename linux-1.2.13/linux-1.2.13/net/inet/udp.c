...
static int udp_deliver(struct sock *sk, struct udphdr *uh, struct sk_buff *skb,
                       struct device *dev, long saddr, long daddr, int len);
...
static int udp_sendto(struct sock *sk, unsigned char *from,
...
static int udp_write(struct sock *sk, unsigned char *buff,
                     int len, int noblock, unsigned flags)
{
...
int udp_recvfrom(struct sock *sk, unsigned char *to,
                 int len, int noblock, unsigned flags,
...
int udp_read(struct sock *sk, unsigned char *buff,
             int len, int noblock, unsigned flags)
{
            ...
            int redo, struct inet_protocol *protocol)
{
    ...
    if (uh->check && udp_check(uh, len, saddr, daddr)) {
        ...
        printk("UDP: bad checksum. From %08lX:%d to %08lX:%d ulen %d\n",
               ...
               ntohl(daddr),ntohs(uh->dest), ulen);
        udp_statistics.UdpInErrors++;
        kfree_skb(skb, FREE_WRITE);
        return(0);
    }
