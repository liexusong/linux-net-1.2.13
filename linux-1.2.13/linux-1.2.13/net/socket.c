...
/*
 * 初始化BSD socket层协议 (比如Unix、INET等)
 */
void proto_init(void)
{
	extern struct net_proto protocols[];	/* Network protocols */ // 在protocols.c文件中
	...
	while (pro->name != NULL)
	{
		(*pro->init_func)(pro); // 如果是Unix就调用unix_proto_init、如果是Inet就调用inet_proto_init
		pro++;
	}
	/* We're all done... */
}
